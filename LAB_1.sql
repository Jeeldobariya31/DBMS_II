...
SELECT C.CourseName , COUNT(E.EnrolmentID) AS TOTAL 
FROM COURSE C 
...
--23.	List all students who are enrolled in more than 3 courses. (STUDENT, ENROLLMENT table)
            SELECT S.StuName , COUNT(E.EnrolmentID)
            FROM STUDENT S
            JOIN ENROLLMENT E
            ON S.StudentID =E.StudentID 
            GROUP BY S.StuName
            HAVING COUNT(E.EnrolmentID) > 3;
...
--25.	Retrieve department-wise count of faculty members along with their average years of experience (calculate experience from JoiningDate). (Faculty table)
         SELECT FacultyDepartment , AVG(DATEDIFF(YEAR,FacultyJoiningDate , GETDATE())) AS experience , COUNT(FacultyID ) AS TOTAL
         FROM FACULTY
